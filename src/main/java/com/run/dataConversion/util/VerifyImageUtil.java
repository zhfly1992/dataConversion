...
public class VerifyImageUtil {
	 ...
	 */
	/*
	 * private static BufferedImage addWatermark(BufferedImage oriImage) throws
	 * IOException { Graphics2D graphics2D = oriImage.createGraphics();
	 * graphics2D
	 * .setRenderingHint(RenderingHints.KEY_INTERPOLATION,RenderingHints
	 * .VALUE_INTERPOLATION_BILINEAR); // 设置水印文字颜色
	 * graphics2D.setColor(Color.BLUE); // 设置水印文字Font graphics2D.setFont(new
	 * java.awt.Font("宋体", java.awt.Font.BOLD, 50)); // 设置水印文字透明度
	 * graphics2D.setComposite
	 * (AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, 0.5f)); //
	 * 第一参数->设置的内容，后面两个参数->文字在图片上的坐标位置(x,y)
	 * graphics2D.drawString("zhoujin@qq.com", 400,300); graphics2D.dispose();
	 * //释放 return oriImage; }
	 */
